...
class Skrllr {
  ...
  addOrphanLinks() {
    ...
    for (let link of links) {
      link.addEventListener(
        "click",
        this.onPaginationClickHandler.bind(this),
        false
      );
    }
  }
  ...
  checkHash() {
    if (
      ...
      window.location.hash !== `#${this.settings.hashPrefix}1`
    ) {
      const index = window.location.hash.replace(
        `#${this.settings.hashPrefix}`,
        ""
      );
      const item = this.el.querySelector(
        `[data-skrllr='${this.settings.hashPrefix}${index}']:not(a)`
      );
      ...
      if (next) {
        ...
        if (this.settings.pagination === true)
          this.settings.menu
            .querySelector(
              `a[data-skrllr='${this.settings.hashPrefix}${index}']`
            )
            .classList.add("activee");

        if (history.replaceState && this.settings.updateURL === true) {
          const href = window.location.href.substr(
            0,
      ...
      this.el
        .querySelector(`[data-skrllr='${this.settings.hashPrefix}1']:not(a)`)
        .classList.add("activee");
      if (this.settings.pagination === true)
        this.settings.menu
          .querySelector(`a[data-skrllr='${this.settings.hashPrefix}1']`)
          .classList.add("activee");
    }
  }
  ...
  onPaginationClickHandler(event) {
    ...
    const index = parseInt(
      event.target.dataset.skrllr.replace(this.settings.hashPrefix, "")
    );
    this.goTo(index);
  }
  ...
  swipeEnd(event) {
    ...
    if (this.elapsedTime <= this.maxAllowedTime) {
      if (
        Math.abs(this.touchEndCoords.y) >= this.minDistanceXAxis &&
        Math.abs(this.touchEndCoords.x) <= this.maxDistanceYAxis
      ) {
  ...
  computeTransition(event, delta) {
    ...
    if (
      timeNow - this.lastTransition <
      this.sleep + this.settings.transitionTime
    ) {
      event.preventDefault();
      return false;
    }
  ...
  goUp() {
    const index = parseInt(
      this.el
        ...
        .dataset.skrllr.replace(this.settings.hashPrefix, "")
    );
    const current = this.el.querySelector(
      `[data-skrllr='${this.settings.hashPrefix}${index}']:not(a)`
    );
    const next = this.el.querySelector(
      `[data-skrllr='${this.settings.hashPrefix}${parseInt(index) - 1}']:not(a)`
    );

    if (next)
      this.position =
        (parseInt(next.dataset.skrllr.replace(this.settings.hashPrefix, "")) -
          1) *
        100 *
        -1;
    else return;
    ...
    const nextIndex = parseInt(
      next.dataset.skrllr.replace(this.settings.hashPrefix, "")
    );
    current.classList.remove("activee");
    next.classList.add("activee");

    if (this.settings.pagination === true) {
      this.settings.menu.querySelector(`.activee`).classList.remove("activee");
      this.settings.menu
        .querySelector(
          `a[data-skrllr='${this.settings.hashPrefix}${nextIndex}']`
        )
        .classList.add("activee");
    }
    ...
    if (history.replaceState && this.settings.updateURL === true) {
      const href =
        ...
        `#${this.settings.hashPrefix}${index - 1}`;
      history.pushState({}, document.title, href);
    }
  ...
  goDown() {
    ...
    const next = this.el.querySelector(
      `[data-skrllr='${this.settings.hashPrefix}${index + 1}']:not(a)`
    );

    if (next) this.position = index * 100 * -1;
    else return;
   ...
   */
  goTo(index) {
    const current = this.el.querySelector(".activee");
    const next = this.el.querySelector(
      `[data-skrllr='${this.settings.hashPrefix}${index}']:not(a)`
    );
    ...
    if (next) {
      ...
      if (this.settings.pagination === true) {
        ...
        this.settings.menu
          .querySelector(`a[data-skrllr='${this.settings.hashPrefix}${index}']`)
          .classList.add("activee");
      }
      ...
      if (history.replaceState && this.settings.updateURL === true) {
        const href =
          window.location.href.substr(0, window.location.href.indexOf("#")) +
          `#${this.settings.hashPrefix}${index}`;
        history.pushState({}, document.title, href);
      }
  ...
  animate(index, nextIndex, next) {
    if (typeof this.settings.beforeTransition === "function")
      this.settings.beforeTransition(index, nextIndex, next);

    const transitionEndHandler = event => {
      event.preventDefault();
      event.target.removeEventListener("transitionend", transitionEndHandler);
      if (typeof this.settings.afterTransition === "function")
        return this.settings.afterTransition(index, nextIndex, next);
    };
